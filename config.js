...
function getChatAccessState(){
  try{const p=JSON.parse(localStorage.getItem('kk_profile_cache')||'null'),year=Number(p?.birth_year||0),age=year?new Date().getFullYear()-year:0;if(!p||p.is_temporary)return{allowed:false,reason:'guest'};if(!year||age<18)return{allowed:false,reason:'minor'};return{allowed:true,reason:''}}catch(e){return{allowed:false,reason:'guest'}}
}
function installChatLockStyles(){if(document.getElementById('kk-chat-lock-style'))return;const st=document.createElement('style');st.id='kk-chat-lock-style';st.textContent=`.kk-chat-locked{opacity:.42!important;filter:grayscale(.72)!important;cursor:not-allowed!important;position:relative!important}.kk-chat-locked::after{content:'🔒';position:absolute;right:5px;top:4px;font-size:11px;line-height:1;background:rgba(7,15,27,.86);border:1px solid rgba(232,201,109,.35);border-radius:999px;padding:4px;z-index:8;box-shadow:0 2px 7px rgba(0,0,0,.3)}#kkChatLockModal{position:fixed;inset:0;z-index:100005;background:rgba(2,8,18,.72);display:flex;align-items:center;justify-content:center;padding:18px;backdrop-filter:blur(8px)}#kkChatLockModal .kk-lock-card{width:min(390px,100%);background:var(--bg2,#10233b);border:1px solid var(--line,rgba(232,201,109,.24));border-radius:22px;padding:22px;color:var(--cream,#f0ead6);text-align:center;box-shadow:0 25px 70px rgba(0,0,0,.5);animation:kkLockIn .22s ease-out}@keyframes kkLockIn{from{opacity:0;transform:translateY(14px) scale(.97)}to{opacity:1;transform:none}}#kkChatLockModal .kk-lock-icon{width:64px;height:64px;margin:0 auto 14px;border-radius:20px;display:grid;place-items:center;font-size:29px;background:rgba(201,168,76,.13);border:1px solid rgba(232,201,109,.3)}#kkChatLockModal h3{margin:0 0 9px;color:var(--gold2,#e8c96d);font-size:20px}#kkChatLockModal p{margin:0;color:var(--muted,#aab8c8);font-size:13px;line-height:1.65}#kkChatLockModal .kk-lock-actions{display:flex;gap:9px;margin-top:19px}#kkChatLockModal button{flex:1;border-radius:12px;padding:11px 12px;font:inherit;font-weight:750;cursor:pointer}#kkChatLockModal .kk-lock-close{color:var(--cream,#eee);background:transparent;border:1px solid var(--line,rgba(255,255,255,.16))}#kkChatLockModal .kk-lock-profile{color:#101722;background:linear-gradient(135deg,#c9a84c,#e8c96d);border:0}`;document.head.appendChild(st)}
function showChatAccessModal(){
  document.getElementById('kkChatLockModal')?.remove();const lang=typeof getAppLanguage==='function'?getAppLanguage():'tg',t=lang==='ru'?{title:'Чат недоступен',text:'Чат недоступен гостям и несовершеннолетним пользователям. Заполните профиль и подтвердите, что вам исполнилось 18 лет.',close:'Закрыть',profile:'Заполнить профиль'}:lang==='en'?{title:'Chat is unavailable',text:'Chat is unavailable to guests and users under 18. Complete your profile and confirm that you are at least 18 years old.',close:'Close',profile:'Complete profile'}:{title:'Чат дастрас нест',text:'Чат барои меҳмонон ва корбарони то 18-сола дастрас нест. Профилро пур кунед ва синни 18-сола ё калонтарро тасдиқ намоед.',close:'Пӯшидан',profile:'Пур кардани профил'};
  const m=document.createElement('div');m.id='kkChatLockModal';m.setAttribute('role','dialog');m.setAttribute('aria-modal','true');m.innerHTML=`<div class="kk-lock-card"><div class="kk-lock-icon">🔒</div><h3>${t.title}</h3><p>${t.text}</p><div class="kk-lock-actions"><button class="kk-lock-close">${t.close}</button><button class="kk-lock-profile">${t.profile}</button></div></div>`;document.body.appendChild(m);const close=()=>m.remove();m.querySelector('.kk-lock-close').onclick=close;m.querySelector('.kk-lock-profile').onclick=()=>{location.href='profile.html?edit=1'};m.onclick=e=>{if(e.target===m)close()};m.querySelector('.kk-lock-close').focus()
}
function applySocialButtonVisibility(){installChatLockStyles();const state=getChatAccessState();document.querySelectorAll('a[href*="chat.html"],a[href*="chats.html"],[onclick*="chat.html"],[onclick*="chats.html"]').forEach(el=>{el.style.display='';el.classList.toggle('kk-chat-locked',!state.allowed);if(!state.allowed){el.setAttribute('aria-disabled','true');el.dataset.chatLocked='1'}else{el.removeAttribute('aria-disabled');delete el.dataset.chatLocked}})}
