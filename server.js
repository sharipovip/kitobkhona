...
async function ensureNotificationsTable() {
  try {
    ...
    console.error('[DB] Failed to ensure notifications table:', e.message);
  }
}

async function ensureNotificationScheduleTable() {
  try {
    await pool.query(`
    ...
    `);
    await pool.query(`ALTER TABLE notification_schedule ADD COLUMN IF NOT EXISTS last_sent_date DATE`);
    console.log('[DB] notification_schedule table ready');
  } catch (e) {
    console.error('[DB] Failed to ensure notification_schedule table:', e.message);
  }
}

async function ensureAchievementNotificationsTable() {
  try {
    await pool.query(`
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_achievement_notifications_user ON achievement_notifications(user_id)`);
    console.log('[DB] achievement_notifications table ready');
  } catch (e) {
    console.error('[DB] Failed to ensure achievement_notifications table:', e.message);
  }
}
...
ensureAchievementNotificationsTable();
...
app.get('/health', (req, res) => {
  res.json({ ok: true, time: new Date().toISOString(), fcm: !!firebaseMessaging });
});
...
// ============ AUTH MIDDLEWARE ============
const DEV_JWT_SECRET = 'kitobkhona-local-development-secret-change-me-2026';
const JWT_SECRET = process.env.JWT_SECRET || (process.env.NODE_ENV === 'production' ? '' : DEV_JWT_SECRET);
if (!JWT_SECRET || JWT_SECRET.length < 32) {
  const message = 'JWT_SECRET must be configured and contain at least 32 characters.';
  if (process.env.NODE_ENV === 'production') {
    console.error('❌ ' + message);
    process.exit(1);
  }
  console.warn('⚠️  ' + message + ' Using the local development secret.');
}
...
app.post('/api/auth/register', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: userId, username: newUser.rows[0].username }, JWT_SECRET, { expiresIn: '30d' });
...
app.get('/api/reading-sessions', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const rows = result.rows.map(row => {
      ...
      return { ...row, folder, file, cover_url: urls.cover_url, pdf_url: urls.pdf_url };
    });
    res.json(rows);
  } catch (e) {
...
app.get('/api/favorites', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const rows = result.rows.map(f => {
      const urls = getBookUrls(f.book_id);
      return { ...f, cover_url: urls.cover_url, pdf_url: urls.pdf_url };
    });
    res.json(rows);
  } catch (e) {
...
app.post('/api/reports', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Отправляем уведомление пользователю о жалобе
    await createUserNotification(reported_user_id, {
      type: 'report',
      title: 'Шикоят',
      body: 'Ба шумо шикоят расид. Лутфан қоидаҳоро риоя кунед.',
      link: '/profile.html'
    });

    try {
      await sendPushToUsers([reported_user_id], {
        title: 'Шикоят',
        body: 'Ба шумо шикоят расид. Лутфан қоидаҳоро риоя кунед.',
        link: '/profile.html',
        data: { type: 'report' }
      ...
      console.warn('[PUSH] Report notification failed:', pushError.message);
    }
...
app.delete('/api/users/me', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await client.query('DELETE FROM reading_sessions WHERE user_id = $1', [userId]);
    ...
    await client.query('DELETE FROM friend_requests WHERE from_user = $1 OR to_user = $1', [userId]);
    await client.query('DELETE FROM friendships WHERE user1_id = $1 OR user2_id = $1', [userId]);
    ...
    await client.query('DELETE FROM player_progress WHERE user_id = $1', [userId]);
    ...
    await client.query('ROLLBACK');
    console.error('Delete own account error:', e.message);
    res.status(500).json({ error: 'Internal server error' });
  } finally {
    client.release();
  }
...
app.post('/api/posts', authenticateToken, async (req, res) => {
  ...
  try {
    // Посты публичные, но публиковать может только пользователь с заполненным профилем.
    // Проверка обязательна на сервере: ограничение нельзя обойти через DevTools или прямой API-запрос.
    ...
    if (missing.length) {
      return res.status(403).json({
        ...
        missing,
        error: 'Барои нашри пост профилро пур кунед: ном, насаб, соли таваллуд ва ҷинс.'
      });
    }
    ...
    const result = await pool.query(
      ...
      [req.user.id, content || '', book_id || null, book_title || null, book_author || null]
    );
...
app.delete('/api/posts/:postId/comments/:commentId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const check = await pool.query(
      'SELECT id FROM post_comments WHERE id = $1 AND user_id = $2 AND post_id = $3',
...
// Публичная пакетная статистика для ленты. Гости могут видеть рейтинги, но не менять их.
app.post('/api/book-stats-batch', async (req, res) => {
  const { book_ids } = req.body;
  if (!Array.isArray(book_ids) || book_ids.length === 0) return res.json({});
  const ids = book_ids.slice(0, 200).map(String);
  try {
    const result = await pool.query(
      `SELECT book_id,
        COUNT(*) FILTER (WHERE reaction = 'like') AS likes,
    ...
    console.error('Batch book stats error:', e.message);
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
app.get('/api/winners', async (req, res) => {
  ...
  try {
    ...
    // Отправляем уведомления для топ-3
...
async function sendAchievementNotifications(topUsers, period) {
  ...
  for (let i = 0; i < topUsers.length; i++) {
    ...
    try {
      // Проверяем, уже отправляли ли уведомление для этого периода и позиции
      const existing = await pool.query(
        `SELECT id FROM achievement_notifications
         WHERE user_id = $1 AND period = $2 AND position = $3`,
        [user.id, period, position]
      );
      ...
      if (existing.rows.length === 0) {
        ...
        await createUserNotification(user.id, {
          type: 'achievement',
          title: 'Табрик мекунам!',
          body: `Шумо сазовари ҷои ${positionName} гаштед!`,
          link: '/winners.html'
        });
        ...
        try {
          await sendPushToUsers([user.id], {
            ...
            body: `Шумо сазовари ҷои ${positionName} гаштед!`,
            link: '/winners.html',
            data: { type: 'achievement', position: String(position) }
          });
        } catch (pushError) {
          console.warn('[PUSH] Achievement notification failed:', pushError.message);
        }

        // Записываем что уведомление отправлено
        await pool.query(
          `INSERT INTO achievement_notifications (user_id, period, position, notified_at)
           VALUES ($1, $2, $3, NOW())`,
          [user.id, period, position]
        );
      }
...
app.post('/api/feedbacks/:id/reply', requireAdmin, async (req, res) => {
  const { id } = req.params;
...
app.post('/api/announcements', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // Отправляем push уведомление всем пользователям
    try {
      const usersResult = await pool.query(`SELECT id FROM users WHERE id != $1`, [req.user?.id || 0]);
      ...
      console.warn('[PUSH] Announcement notification failed:', pushError.message);
    }
...
async function sendPushMessagesToTokens(tokens, { title, body, link, data } = {}) {
  ...
  const message = {
    ...
    android: {
      notification: {
        sound: 'default',
        clickAction: 'FLUTTER_NOTIFICATION_CLICK' // Совместимость с кликами
      }
    },
    webpush: {
...
app.get('/api/debug/db', requireAdmin, async (req, res) => {
  try {
    const r = await pool.query('SELECT NOW() as now, COUNT(*) as users FROM users');
    res.json({ ok: true, db_time: r.rows[0].now, users: parseInt(r.rows[0].users) });
